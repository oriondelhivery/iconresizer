// script.js -- Manual-generate + Unmasked (final)
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleFile(file) {
    ...
    reader.onload = (event) => {
      ...
      img.onerror = () => { uploadedImage = null; };
      img.src = event.target.result;
    };
    reader.readAsDataURL(file);
  }

  // Previews: square and circular canvases
  ...
  // Generate ZIP only when button clicked (manual flow)
  generateBtn.addEventListener("click", async () => {
    if (!uploadedImage) return; // nothing to do
    ...
    for (const size of allSizes) {
      ...
      tmpCtx.closePath();
      tmpCtx.clip();
      tmpCtx.drawImage(uploadedImage, 0, 0, size, size);
